...
function PipelineSkeleton() {
  return (
    <div className="col-span-2 bg-card/50 border border-border rounded-2xl p-6 h-96 animate-pulse"></div>
  );
}

function CallsSkeleton() {
  return (
    <div className="col-span-1 bg-card/50 border border-border rounded-2xl p-6 h-96 animate-pulse"></div>
  );
}
  ...
  return (
    <div className="mt-8 animate-pulse">
      <div className="flex items-center justify-between mb-6">
        <div className="h-6 w-32 bg-card/50 rounded"></div>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[1, 2, 3].map(i => (
          <div key={i} className="rounded-2xl border border-border bg-card/50 h-64"></div>
        ))}
      </div>
    </div>
  );
...
async function QuickStats({ dict }: { dict: any }) {
  const [activePropertiesCount, newLeadsCount, scheduledAppointmentsCount, closedSalesAggregate] = await Promise.all([
...
async function PipelineData({ dict }: { dict: any }) {
  ...
  return (
    <div className="col-span-2 bg-card border border-border rounded-2xl p-6 h-full">
      <div className="flex items-center justify-between mb-6">
...
async function CallsData({ dict }: { dict: any }) {
  ...
  return (
    <div className="col-span-1 bg-card border border-border rounded-2xl p-6 flex flex-col h-full">
      <div className="flex items-center justify-between mb-6">
...
async function PropertiesData({ dict }: { dict: any }) {
