    ...
    <div id="main-scroll-container" className="flex-1 flex flex-col h-full bg-background overflow-auto">
      ...
      {showShareModal && (
        <div className="fixed inset-0 z-[100] bg-black/60 flex items-center justify-center p-4" onClick={() => setShowShareModal(false)}>
           <div className="bg-background rounded-3xl p-8 max-w-sm w-full relative flex flex-col items-center text-center shadow-2xl" onClick={e => e.stopPropagation()}>
              <button onClick={() => setShowShareModal(false)} className="absolute top-4 right-4 p-2 hover:bg-muted rounded-full">
                 <X className="w-5 h-5" />
              </button>
              <h3 className="text-xl font-bold mb-2">Compartir Propiedad</h3>
              <p className="text-sm text-muted-foreground mb-8">
                 Escanea este código para ver la página pública de la propiedad en tu celular.
              </p>
              
              <div className="bg-white p-4 rounded-xl shadow-inner mb-6">
                 <QRCodeSVG value={`${window.location.origin}/p/${property.id}`} size={200} />
              </div>
              ...
              <button 
                 ...
                 className="w-full py-3 bg-muted hover:bg-muted/80 font-bold rounded-xl flex items-center justify-center gap-2"
              >
