    ...
    <div className="p-8 overflow-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4 animate-in">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Gestiona el inventario de propiedades exclusivas para renta.</p>
        </div>
        <div className="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto mt-4 md:mt-0">
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            ...
            >
              <Plus className="w-4 h-4" /> Subir Renta
            </Link>
          </div>
        </div>
      </div>
      ...
      <div className="flex gap-4 mb-8 border-b border-border overflow-x-auto pb-px">
        {['TODOS', 'CASA', 'DEPARTAMENTO'].map(cat => (
          <button 
            key={cat}
          ...
          >
            {cat === 'TODOS' ? dict.propertiesPage.tabs.all : cat === 'CASA' ? dict.propertiesPage.tabs.houses : dict.propertiesPage.tabs.apartments}
          </button>
        ))}
      </div>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center h-64 text-center border-2 border-dashed border-border rounded-2xl bg-card p-8 animate-in fade-in zoom-in-95 duration-300">
          ...
          <h3 className="text-xl font-bold mb-2">No hay rentas en esta sección</h3>
          <p className="text-muted-foreground mb-6 max-w-md">
            Actualmente no hay inmuebles en renta registrados aquí.
          </p>
          <Link 
            href="/admin/rentas/new"
